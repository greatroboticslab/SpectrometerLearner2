...
def MakeDataSet(dataDir):
    ...
    #Returns wavelength inputs (and the sent wavelength) as toConvert, and the labels as toConvertTargets
    
    return (toConvert, toConvertTargets)

def MakeNumpySet(dataDir):
    toConvert, toConvertTargets = MakeDataSet(dataDir)
    entryCount = len(toConvert)
    
    wCnt = len(toConvert[0])
    
    inputs = np.zeros((entryCount, wCnt))
    targets = np.zeros((entryCount))
    
    for x in range(entryCount):
        inputs[x] = toConvert[x]
        targets[x] = toConvertTargets[x]
        
    return (inputs, targets)
    
def MakeNumpySetClipped(dataDir, threshold, size):
    ...
    for entry in toConvert:
        ...
        added = 0
        wPos = 0
        for intensity in entry:
            if intensity >= threshold and added < size:
                clip.append(intensity)
                added += 1
                posClip.append(wPos)
            wPos += 1
        while(added < size):
            ...
            clip.append(0.0)
            posClip.append(-1)
        toConvertClipped.append(clip)
        toConvertPositions.append(posClip)
    
    wCnt = size
    
    inputs = np.zeros((entryCount, 2, wCnt))
    targets = np.zeros((entryCount))
    
    for x in range(entryCount):
        inputs[x][0] = toConvertClipped[x]
        inputs[x][1] = toConvertPositions[x]
        targets[x] = toConvertTargets[x]
        
    return (inputs, targets)
    
def MakeTorchDataSet(dataDir):
    toConvert, toConvertTargets = MakeDataSet(dataDir)
